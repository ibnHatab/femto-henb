...
typedef enum {
    ...
    FAPI_L1_SRS_BandwidthConfiguration = 29,
    ...
    //    FAPI_L1_CONFIG_TLV_MAX = FAPI_L1_PHY_State + 1,
...
typedef struct l1_tlv_word l1_tlv_word_t;

typedef enum {
    FAPI_TLV_NOERROR      = 0x00,
    FAPI_TLV_MISSING      = 0x01,
    FAPI_TLV_WRONG_VALUE  = 0x02,
    FAPI_TLV_UNSUPPORTED  = 0x04,
} cl_conv_error_e;


/** 
 * Linear search for TLV in array by its TAG
 * 
 * @param tlv    - tag to find
 * @param array  - array
 * @param number - array length
 * 
 * @return tlv index in array
 */
