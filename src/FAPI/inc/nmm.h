...
enum FAPI_NNM_MessageTypeID_e {
  NMM_RSSI_MEAS_REQ        =  1, // 6.1.1 Triggers RSSI measurements
                                 // in the requested band. Optionally a list of carriers can be configured.
  NMM_RSSI_MEAS_IND        =  2, // 6.1.2  Returns the measurement on a carrier.
  NMM_RSSI_MEAS_CNF        =  3, // 6.1.3  Indicates the end of the measurements.
  NMM_STOP_RSSI_MEAS_REQ   =  4, // 6.1.4  Stops the RSSI measurements.
  NMM_CELL_SEARCH_REQ      =  5, // 6.1.5  Triggers the search for cells on the
                                 // specified carrier. Optionally a list of cells can be specified.
  NMM_CELL_SEARCH_IND      =  6, // 6.1.6  Returns a found cell.
  NMM_CELL_SEARCH_CNF      =  7, // 6.1.7  Indicates the end of the cell search.
  NMM_STOP_CELL_SEARCH_REQ =  8, // 6.1.8  Stops the cell search.
  NMM_PBCH_CONFIG_REQ      =  9, // 6.1.9  Triggers reading of the PBCH.
...
struct VendorSpecificListElement {
  ...
  uint8_t  value[]; //  struct  The actual value which will be set
};
